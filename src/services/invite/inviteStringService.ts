...
/**
 * We now have three distinct invite types:
 *
 * 1. prelaunch is generated on the backend, because user has not yet generated
 *    master-password and therefore has no public/private keys. This invite is not
 *    limited in spread, so no `inviteId` is present in the payload. It will only
 *    work for prelaunch sign up process and will turn into ashes afterwards.
 *
 * 2. service invite. Is generated by a signed up user with their keys, is a one-time
 *    use thing (has `inviteId`) and should be limited in quantity.
 *
 * 3. wallet invite. All the same as service invite, but also launches the wallet join
 *    process.
 *
...
export class InviteStringService {
  public static async parseAndVerifySignature(
    ...
    if (this.isAesInvite(inviteString)) {
      try {
        const { userInviterId } = decryptAes<PrelaunchInvitePayload>(
          inviteString,
        )
        runSchemaWithFormError(requiredString, userInviterId)
        return { type: InviteStringTypes.prelaunch, payload: { userInviterId } }
      } catch (e) {
        throw new FormValidationError(InviteServiceFormErrors.invalidInvite)
      }
    }
  ...
  public static generatePrelaunchInvite(userId: string): string {
    const payload: PrelaunchInvitePayload = { userInviterId: userId }
