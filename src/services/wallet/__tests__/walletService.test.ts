...
describe('Wallet Service', () => {
  ...
  describe('add user', () => {
    ...
    it('works', async () => {
      ...
      expect(mockWalletAccessManager.addUser.mock.calls.length).toBe(1)
      const {
        userId: _userId,
        inviteId: _inviteId,
        walletId: _walletId,
      } = mockWalletAccessManager.addUser.mock.calls[0][0]
      expect(_userId).toBe(userToAddId)
      expect(_inviteId).toBe(inviteId)
      expect(_walletId).toBe(walletId)
      ...
      expect(
        mockWalletPubSubService.publishWalletUpdates.mock.calls[0][0].wallet.id,
      ).toBe(walletId)
    })
    ...
    it('throws if such wallet does not exist', async () => {
      mockWalletManager.byIdAndUserId.mockImplementationOnce(
        async () => undefined,
      )
      try {
        await WalletService.addUser({
    ...
    it('does not add user that is already a part of the wallet', async () => {
      await WalletService.addUser({
        initiatorId,
        walletId,
        userToAddId: initiatorId,
        inviteId,
      })
      expect(mockWalletAccessManager.addUser.mock.calls.length).toBe(0)
